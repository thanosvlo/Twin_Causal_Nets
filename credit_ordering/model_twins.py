...
def multi_class_twin_za(treatment, uy, confounders, params):
    lattice_sizes = params.lattice_sizes
    model_inputs = []
    ...
    if params.concats:
        ...
        fact_out = []
        counterfact_out = []
        for ii in np.arange(params.target_min, params.target_max+1, 1):
            lattice_y = tfl.layers.Lattice(
            ...
            lattice_y_prime = tfl.layers.Lattice(
                ...
                output_min=0,
                output_max=1,
                name='Y_prime_{}'.format(ii),
            )([a_prime_calibrator, layer_uy, *layer_z])
            counterfact_out.append(lattice_y_prime)
    ...
    model = tf.keras.models.Model(
        inputs=model_inputs,
        outputs=[*fact_out, *counterfact_out])
    return  model
