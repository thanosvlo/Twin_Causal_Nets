...
#
#     predictions_logit = predictions[:,1]#np.array([logit(xi) for xi in predictions[:, 1]])
#
#     X.loc[:, 'propensity_score'] = predictions[:, 1]
#     X.loc[:, 'propensity_score_logit'] = predictions_logit
#     X.loc[:, 'Y'] = dataset_base.Y
#     X.loc[:, 'X'] = dataset_base.X
...
#
#     X['X_prime'] = np.logical_xor(X['X'],1).astype(int)
#     X = X.reset_index()
...
#
#     X.insert(1, 'Y_prime', temp)
#     X.drop(['index'],axis=1,inplace=True)
#
#
#     Xs = X.train['X'].values
#     Xs_prime = X.train['X_prime'].values
#     Ys = X.train['Y'].values
#     Ys_prime = X.train['Y_prime'].values
