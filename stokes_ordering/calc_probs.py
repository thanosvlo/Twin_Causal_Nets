...
def calc_acc(gt_fact,gt_counter,preds,args):
    ...
    if len(np.unique(gt_counter.values)) > 2:
        f1 = f1_score(gt_counter.iloc[idx_given_cor],pred_counter[idx_given_cor],average='macro')
    else:
...
def run_inference(args):
    ...
    calc_acc(target_test,target_prime_test,preds,args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--inference_name',
                       # default='twin_net_arch_azlink_lattice_treat_monot_increasing_none_uy_none_uy_monot_none_z_monot_opt_1_z_layer_multiple_none_calib_units_4_4_z_4_lr_0_001_loss_mse_Stroke__confounders_all')
                       # default='twin_net_arch_azlink_lattice_treat_monot_increasing_none_uy_none_uy_monot_none_z_monot_opt_1_z_layer_multiple_none_calib_units_4_4_z_4_lr_0_001_loss_mse_Stroke_2__confounders_all')
                       # default='twin_net_arch_azlink_lattice_treat_monot_none_none_uy_none_uy_monot_none_z_monot_opt_1_z_layer_multiple_none_calib_units_4_4_z_4_lr_0_001_loss_mse_Stroke_2__confounders_all')
                       # default='twin_net_arch_azlink_linear_concat_treat_monot_none_none_uy_none_uy_monot_none_z_monot_opt_1_z_layer_multiple_none_calib_units_4_4_z_4_lr_0_001_loss_mse_Stroke_2__confounders_all') #USED IN PAPER
                       default='twin_net_arch_azlink_lattice_concat_treat_monot_increasing_none_uy_none_uy_monot_none_z_monot_opt_1_z_layer_multiple_none_calib_units_4_4_z_4_lr_0_001_loss_mse_Stroke_heparin__confounders_all') # USED IN PAPER
    # Logging
    parser.add_argument('--restore', type=bool, default=False)
    parser.add_argument('--log_root', type=str, default='./experiments/Stroke_treat_{}_outcome_{}/')
    parser.add_argument('--name', type=str,
                        default='twin_net_arch_{}_{}_uy_{}_uy_monot_{}_z_monot_{}_z_layer_{}_calib_units_{}_z_{}_lr_{}_loss_{}_Stroke_{}_confounders_{}')
    ...
    parser.add_argument('--bins', default=3, help='quantization bins of outcome')
    ...
    if args.analysis:
        ...
        for treat, treat_counter in itertools.permutations(np.unique(dataset.test[args.treatment]), 2):
            if len(np.unique(dataset.test[args.outcome])) > 2:
                ...
                x_pn = PrettyTable(
                    ['PN_{}_{}'.format(treat, treat_counter), *[str(i) for i in np.unique(dataset.test[args.outcome])]])
                x_ps = PrettyTable(
                    ['PS_{}_{}'.format(treat, treat_counter), *[str(i) for i in np.unique(dataset.test[args.outcome])]])
                x_pns = PrettyTable(['PNS_{}_{}'.format(treat, treat_counter),
                                     *[str(i) for i in np.unique(dataset.test[args.outcome])]])
                x_ate = PrettyTable(['ATE_{}_{}'.format(treat, treat_counter),
                                     *[str(i) for i in np.unique(dataset.test[args.outcome])]])

                res_1 = np.zeros((int(dataset.test[args.outcome].max() + 2), int(dataset.test[args.outcome].max() + 2)),
                                 dtype=object)
                res_2 = np.zeros((int(dataset.test[args.outcome].max() + 2), int(dataset.test[args.outcome].max() + 2)),
                                 dtype=object)
                res_3 = np.zeros((int(dataset.test[args.outcome].max() + 2), int(dataset.test[args.outcome].max() + 2)),
                                 dtype=object)
                res_4 = np.zeros((int(dataset.test[args.outcome].max() + 2), int(dataset.test[args.outcome].max() + 2)),
                                 dtype=object)

                res_1[0, :] = ['PN_{:.0f}_{:.0f}'.format(treat, treat_counter),
                               *[str(i) for i in np.unique(dataset.test[args.outcome])]]
                res_2[0, :] = ['PS_{:.0f}_{:.0f}'.format(treat, treat_counter),
                               *[str(i) for i in np.unique(dataset.test[args.outcome])]]
                res_3[0, :] = ['PNS_{:.0f}_{:.0f}'.format(treat, treat_counter),
                               *[str(i) for i in np.unique(dataset.test[args.outcome])]]
                res_4[0, :] = ['ATE_{:.0f}_{:.0f}'.format(treat, treat_counter),
                               *[str(i) for i in np.unique(dataset.test[args.outcome])]]

                for outcome_cond, outcome_counter in itertools.permutations(np.unique(dataset.test[args.outcome]), 2):
                    res_1[int(outcome_cond) + 1, int(outcome_counter) + 1] = "${:.4f} \pm {:.4f} $".format(np.array(
                        cumulative_results[str(treat)][str(treat_counter)][str(outcome_cond)][str(outcome_counter)][
                            'PN']).mean(), np.array(
                        cumulative_results[str(treat)][str(treat_counter)][str(outcome_cond)][str(outcome_counter)][
                            'PN']).std())
                    res_2[int(outcome_cond) + 1, int(outcome_counter) + 1] = "${:.4f} \pm {:.4f} $".format(np.array(
                        cumulative_results[str(treat)][str(treat_counter)][str(outcome_cond)][str(outcome_counter)][
                            'PS']).mean(), np.array(
                        cumulative_results[str(treat)][str(treat_counter)][str(outcome_cond)][str(outcome_counter)][
                            'PS']).std())
                    res_3[int(outcome_cond) + 1, int(outcome_counter) + 1] = "${:.4f} \pm {:.4f} $".format(np.array(
                        cumulative_results[str(treat)][str(treat_counter)][str(outcome_cond)][str(outcome_counter)][
                            'PNS']).mean(), np.array(
                        cumulative_results[str(treat)][str(treat_counter)][str(outcome_cond)][str(outcome_counter)][
                            'PNS']).std())
                    res_4[int(outcome_cond) + 1, int(outcome_counter) + 1] = "${:.4f} \pm {:.4f} $".format(np.array(
                        cumulative_results[str(treat)][str(treat_counter)][str(outcome_cond)][str(outcome_counter)][
                            'ATE']).mean(), np.array(
                        cumulative_results[str(treat)][str(treat_counter)][str(outcome_cond)][str(outcome_counter)][
                            'ATE']).std())
            ...
            print('\\caption{Probability of Necessity -- columns and rows are outcomes -- given treatment ' + str(
                treat) + ' counter treatment ' + str(treat_counter) + '}')
            print('\\end{table}')
            print(' ')
            if gt:
                print('\\begin{table}')
                ...
                print(
                    '\\caption{Probability of Necessity -- Ground Truth-- columns and rows are outcomes -- given treatment ' + str(
                        treat) + ' counter treatment ' + str(treat_counter) + '}')
                print('\\end{table}')
            print(' ')
            print('\\begin{table}')
            ...
            print(
                '\\caption{Probability of Sufficiency -- columns and rows are outcomes -- given treatment ' + str(
                    treat) + ' counter treatment ' + str(treat_counter) + '}')
            print('\\end{table}')
            ...
            if gt:
                ...
                print(
                    '\\caption{Probability of Sufficiency -- Ground Truth-- columns and rows are outcomes-- given treatment ' + str(
                        treat) + ' counter treatment ' + str(treat_counter) + '}')
                print('\\end{table}')
                print(' ')
            print('\\begin{table}')
            ...
            print(
                '\\caption{Probability of Necessity \& Sufficiency -- columns and rows are outcomes -- given treatment ' + str(
                    treat) + ' counter treatment ' + str(treat_counter) + '}')
            print('\\end{table}')
            print(' ')
            if gt:
                ...
                print(
                    '\\caption{Probability of Necessity \& Sufficiency -- Ground Truth -- columns and rows are outcomes -- given treatment ' + str(
                        treat) + ' counter treatment ' + str(treat_counter) + '}')
                print('\\end{table}')
            print(' ')
            print('\\begin{table}')
            ...
            print(
                '\\caption{ ATE -- columns and rows are outcomes --- given treatment ' + str(
                    treat) + ' counter treatment ' + str(treat_counter) + '}')
            print('\\end{table}')
